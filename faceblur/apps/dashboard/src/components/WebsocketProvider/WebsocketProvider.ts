...
import {ClientMessage, ClientMessageType, ServerMessage, ServerMessageType} from "../../proto/websocket/v1/websocket";
import {createContext, createElement, PropsWithChildren, useContext, useEffect, useMemo, useRef} from "react";

type WebsocketProviderProps = Record<string, unknown>;

export type EventListener = (message: ServerMessage) => void;
type AttachEventListener = (type: ServerMessageType, listener: EventListener) => void;
...
export const WebsocketProvider = ({ children }: PropsWithChildren<WebsocketProviderProps>) => {
	...
	const value = useMemo<ContextValue>(() => {
		return {
			...
			removeEventListener: (type, cb) => {
				...
				listeners.current.set(type, listeners.current.get(type)?.filter((savedCB) => savedCB !== cb) ?? []);
			},
		};
	}, [listeners]);
	...
	useEffect(() => {
		...
		if (lastJsonMessage) {
			listeners.current.get(message.type)?.forEach((listener) => {
				listener(message);
			});
		}
