...
func (s *Service) Detect(image image.Image) ([]*domain.BoundingBox, error) {
	originalWidth := image.Bounds().Canon().Dx()
	originalHeight := image.Bounds().Canon().Dy()

	modelSession, e := s.initSession()
	if e != nil {
		return nil, syserr.Wrap(e, "could not create session")
	}
	...
	boundingBoxes := s.processOutput(modelSession.Output.GetData(), originalWidth, originalHeight)
...
func (s *Service) processOutput(output []float32, originalWidth, originalHeight int) []utilImage.BoundingBox {
	...
	for idx := 0; idx < detectionCount; idx++ {
		if idx >= len(output) || 8400+idx >= len(output) || 2*8400+idx >= len(output) || 3*8400+idx >= len(output) {
			s.loggerService.Info(context.TODO(), "skipping idx out of bounds", logger.F("idx", idx))
			continue
		}
